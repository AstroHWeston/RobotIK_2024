...
tinyNeoPixel::tinyNeoPixel()
    : begun(false), numLEDs(0), numBytes(0), pin(NOT_A_PIN), brightness(0), pixels(NULL), rOffset(1), gOffset(0),
...
void tinyNeoPixel::begin(uint8_t _addr)
{
    addr = _addr;
    Wire.begin();
    native = 0;
...
void tinyNeoPixel::sendColorI2C(uint8_t r, uint8_t g, uint8_t b)
{
...
// 5ish MHz(ish) AVRxt
    ...
    asm volatile(
        ...
        : [ptr] "+e"(ptr), [byte] "+r"(b), [n1] "+r"(n1), [n2] "+r"(n2),
          [count] "+d"(i) /* Relax requirements - it only needs to be an upper register, because we can't use SBIW,
                             because we need to split the subtraction between two bits to meet timing constraints) */
...
// 6.4ish MHz(ish) AVRxt
    ...
    asm volatile("headD:"
                 ...
                 : [ptr] "+e"(ptr), [byte] "+r"(b), [n1] "+r"(n1), [n2] "+r"(n2), [count] "+w"(i)
                 : [port] "e"(port), [hi] "r"(hi), [lo] "r"(lo));
...
// 8 MHz(ish) AVRxt ---------------------------------------------------------
    ...
    */
    ...
    asm volatile("headD:"
                 ...
                 "doneD:"
                 "\n"
                 : [ptr] "+e"(ptr), [byte] "+r"(b), [next] "+r"(next), [count] "+w"(i)
                 : [port] "e"(port), [hi] "r"(hi), [lo] "r"(lo));
...
// 16 MHz(ish) AVRxt ------------------------------------------------------
    ...
    asm volatile("head20:"
                 ...
                 : [ptr] "+e"(ptr), [byte] "+r"(b), [bit] "+d"(bit), [next] "+r"(next), [count] "+w"(i)
                 : [port] "e"(port), [hi] "r"(hi), [lo] "r"(lo));
...
#error "CPU SPEED NOT SUPPORTED"
    ...
    interrupts();
#if (!defined(MILLIS_USE_TIMERNONE) && !defined(MILLIS_USE_TIMERRTC) && !defined(MILLIS_USE_TIMERRTC_XTAL) &&          \
     !defined(MILLIS_USE_TIMERRTC_XOSC))
    endTime = micros();
    // Save EOD time for latch on next call
#else
#warning                                                                                                               \
    "micros() is not available because millis is disabled from the tools subemnu. It is your responsibility to ensure a sufficient time has passed between calls to show(). See documentation."
#endif
}
